...
// The observer set is only touched on the main loop; the ffrt::mutex guards stay so that host
// self-tests flag any access from elsewhere.
...
void CallStateListener::MeeTimeStateUpdated(CallAttributeInfo info, TelCallState priorState, TelCallState nextState)
{
    std::lock_guard<ffrt::mutex> lock(mutex_);
    for (auto &observer : listenerSet_) {
